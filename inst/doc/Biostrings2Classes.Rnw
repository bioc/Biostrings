...
and one should always use the \Rmethod{subXString} method to extract a substring
from a big string:
<<a4>>=
bb <- subXString(b, 3, 6)
dd1 <- subXString(d, end=7)
dd2 <- subXString(d, start=8)
@
...
XString objects. It will be essentially doing toString(subXString()).]}
...
is equivalent to \Robject{subXString(b, start[i], end[i])}.
...
v12[0]
v12[FALSE]
Views(d, start=integer(0), end=integer(0))
@
